"""
Settings Service
----------------
Manages company profile settings and tax rate lookups.

Password protection (optional):
  - If settings_password_hash is NULL → settings are freely editable
  - If set → PATCH /settings requires X-Settings-Password header
  - Password stored as SHA-256 hex digest (no salt — MVP simplicity)
  - For production: replace with bcrypt

Tax rates: hardcoded dict of country ISO code → standard VAT/GST/sales tax %.
  When frontend selects country, default_tax_rate auto-fills from this dict.
"""

import hashlib
import logging
from datetime import datetime

from supabase import Client

from app.core.exceptions import DatabaseError, NotFoundError, ValidationError

log = logging.getLogger(__name__)


# ---------------------------------------------------------------------------
# Country → standard tax rate (%)
# ---------------------------------------------------------------------------

COUNTRY_TAX_RATES: dict[str, float] = {
    "US": 0.0,   # no federal VAT; state sales tax varies
    "UK": 20.0,
    "DE": 19.0,
    "FR": 20.0,
    "TR": 20.0,
    "SA": 15.0,
    "AE": 5.0,
    "EG": 14.0,
    "BH": 10.0,
    "DK": 25.0,
    "FI": 24.0,
    "IT": 22.0,
    "ES": 21.0,
    "NL": 21.0,
    "SE": 25.0,
    "NO": 25.0,
    "JP": 10.0,
    "KR": 10.0,
    "CN": 13.0,
    "IN": 18.0,
    "BR": 17.0,
    "CA": 13.0,
    "AU": 10.0,
    "CY": 19.0,
    "PL": 23.0,
    "PT": 23.0,
    "CH": 7.7,
    "SG": 9.0,
    "MY": 6.0,
    "ZA": 15.0,
    "NG": 7.5,
    "MX": 16.0,
    "AR": 21.0,
}


# ---------------------------------------------------------------------------
# Read
# ---------------------------------------------------------------------------

def get_company_settings(db: Client, company_id: str) -> dict:
    """
    Returns company profile settings.
    Strips settings_password_hash from the response — never expose it.
    """
    try:
        result = (
            db.table("companies")
            .select(
                "id, name, domain, country, default_tax_rate, "
                "address, phone, email, tax_id, logo_url, created_at"
            )
            .eq("id", company_id)
            .single()
            .execute()
        )
    except Exception as e:
        raise DatabaseError("Failed to fetch company settings", detail=str(e))

    if not result.data:
        raise NotFoundError(f"Company {company_id} not found")

    return result.data


# ---------------------------------------------------------------------------
# Update
# ---------------------------------------------------------------------------

def update_company_settings(
    db: Client, company_id: str, data: dict
) -> dict:
    """
    Update company profile settings.
    Writes audit log on change.
    Caller is responsible for password verification before calling this.
    """
    if not data:
        return get_company_settings(db, company_id)

    # Fetch current for audit
    current = get_company_settings(db, company_id)

    # Sanitize — never allow these fields to be updated here
    data.pop("id", None)
    data.pop("created_at", None)
    data.pop("settings_password_hash", None)

    try:
        result = (
            db.table("companies")
            .update(data)
            .eq("id", company_id)
            .execute()
        )
    except Exception as e:
        raise DatabaseError("Failed to update company settings", detail=str(e))

    if not result.data:
        raise NotFoundError(f"Company {company_id} not found")

    # Audit log
    try:
        db.table("audit_logs").insert({
            "company_id":   company_id,
            "table_name":   "companies",
            "record_id":    company_id,
            "action":       "update",
            "performed_by": None,
            "old_data":     current,
            "new_data":     data,
        }).execute()
    except Exception as e:
        log.error(f"Audit log failed for settings update: {e}")

    return get_company_settings(db, company_id)


# ---------------------------------------------------------------------------
# Tax rates
# ---------------------------------------------------------------------------

def get_tax_rates() -> dict:
    """Returns the full country → tax rate mapping."""
    return COUNTRY_TAX_RATES


def get_tax_rate_for_country(country_code: str) -> float | None:
    """Returns tax rate for a specific country code, or None if not found."""
    return COUNTRY_TAX_RATES.get(country_code.upper())


# ---------------------------------------------------------------------------
# Password management
# ---------------------------------------------------------------------------

def _hash_password(password: str) -> str:
    return hashlib.sha256(password.encode()).hexdigest()


def set_settings_password(
    db: Client, company_id: str, password: str
) -> dict:
    """
    Set or update the settings password.
    Passing an empty string clears the password (open access).
    """
    if not password:
        # Clear password protection
        hashed = None
    else:
        if len(password) < 6:
            raise ValidationError("Settings password must be at least 6 characters")
        hashed = _hash_password(password)

    try:
        db.table("companies").update({
            "settings_password_hash": hashed,
        }).eq("id", company_id).execute()
    except Exception as e:
        raise DatabaseError("Failed to set settings password", detail=str(e))

    # Audit
    try:
        db.table("audit_logs").insert({
            "company_id":   company_id,
            "table_name":   "companies",
            "record_id":    company_id,
            "action":       "update",
            "performed_by": None,
            "old_data":     None,
            "new_data":     {"settings_password_changed": True},
        }).execute()
    except Exception as e:
        log.error(f"Audit log failed for password change: {e}")

    return {
        "success": True,
        "message": "Password cleared" if not password else "Password set successfully",
        "password_protected": bool(hashed),
    }


def verify_settings_password(
    db: Client, company_id: str, password: str
) -> bool:
    """
    Returns True if password matches stored hash.
    Returns True if no password is set (open access).
    Returns False if password is wrong.
    """
    try:
        result = (
            db.table("companies")
            .select("settings_password_hash")
            .eq("id", company_id)
            .single()
            .execute()
        )
    except Exception as e:
        raise DatabaseError("Failed to verify password", detail=str(e))

    if not result.data:
        raise NotFoundError(f"Company {company_id} not found")

    stored_hash = result.data.get("settings_password_hash")

    # No password set → open access
    if not stored_hash:
        return True

    return _hash_password(password) == stored_hash


def is_password_protected(db: Client, company_id: str) -> bool:
    """Returns True if the company has a settings password set."""
    try:
        result = (
            db.table("companies")
            .select("settings_password_hash")
            .eq("id", company_id)
            .single()
            .execute()
        )
        return bool(result.data and result.data.get("settings_password_hash"))
    except Exception:
        return False


# ---------------------------------------------------------------------------
# Pipeline config (read-only, from app config)
# ---------------------------------------------------------------------------

def get_pipeline_config() -> dict:
    """
    Returns read-only pipeline configuration from app settings.
    Safe to expose to frontend — no secrets.
    """
    from app.core.config import get_settings
    settings = get_settings()
    return {
        "llm_provider":      settings.LLM_PROVIDER,
        "llm_model":         settings.OPENAI_MODEL if settings.LLM_PROVIDER == "openai" else settings.OLLAMA_MODEL,
        "embedding_model":   settings.OPENAI_EMBEDDING_MODEL if settings.LLM_PROVIDER == "openai" else settings.OLLAMA_EMBEDDING_MODEL,
        "embedding_dimension": settings.EMBEDDING_DIMENSION,
        "ocr_engine":        "PyMuPDF + Tesseract fallback",
        "tesseract_path":    settings.TESSERACT_PATH or "not configured",
        "rerank_model":      settings.COHERE_RERANK_MODEL,
        "chunk_size_tokens": settings.DOCUMENT_CHUNK_SIZE_TOKENS,
        "chunk_overlap_tokens": settings.DOCUMENT_CHUNK_OVERLAP_TOKENS,
    }